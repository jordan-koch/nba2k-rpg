...
const FINDINGS_BY_LENS = {
  fidelity: [
    { loc: 'src/nba_platform/land/writer.py:42', title: 'landed payload is overwritten instead of written once' },
    { loc: 'transform/models/silver/fact_player_game.sql:88', title: 'declared grain has no uniqueness test in schema.yml' },
    { loc: 'src/nba_platform/extract/client.py:120', title: 'plan phase 3 was never implemented at all' },
  ],
  correctness: [
    { loc: 'src/nba_platform/land/writer.py:42', title: 'landed payload is overwritten, not written once' },  // dup of fidelity[0]
  ...
  extraction: [
    { loc: 'src/nba_platform/land/writer.py:60', title: 'no checkpoint, so a failed backfill restarts from zero' },       // same file, different bug
  ],
  'skill-quality': [
...
// ── Scenario 1: CAP + DEDUPE + COVERAGE ─────────────────────────────────────────
{
  ...
  const writerFindings = vf.filter(f => String(f.location).startsWith('src/nba_platform/land/writer.py'))
  ...
  for (const [file, group] of [['src/nba_platform/land/writer.py', writerFindings], ['transform/models/silver/dim_player.sql', dimPlayerFindings]]) {
