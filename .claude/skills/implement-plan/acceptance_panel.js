...
const READONLY = "YOU ARE READ-ONLY-BUT-VERIFICATION-CAPABLE. You MAY read/grep any file and you MAY RUN read-only verification commands to ground your judgment: 'uv run pytest', 'uv run ruff check', 'uv run mypy', the domain core against a committed fixture, 'git diff HEAD', 'git status', 'git log'. You MUST NOT modify any file (no Edit/Write) and MUST NOT run any git command that changes the working tree or history (no checkout/reset/restore/clean/stash/commit/add/rm). Do NOT run anything that writes to var/, hits a live external source (CI excludes network tests for that reason), or reads/writes a real career under careers/ — use tests/fixtures/. If a change is needed, DESCRIBE it as a finding — never make it. (A write-capable review agent once ran 'git checkout' and silently reverted all uncommitted work; that is why this rule is absolute.)"

const SHARED = `You are REVIEWING a COMPLETED implementation of a DECIDED plan, for an RPG progression engine for NBA 2K26 MyNBA player-locked careers: box scores are entered by hand, an expectation-relative economy converts them to currency, and the player spends it on attributes and badges priced by their build. Python domain core (src/rpg_core) + FastAPI + React. Careers are an append-only event ledger folded into state. Working directory = repo root. The code has ALREADY been written and sits UNCOMMITTED in the working tree.
...
- CLAUDE.md — the conventions: resolve datasets BY NAME via datasets/manifest.json (never a literal path); the LEDGER IS APPEND-ONLY (code that mutates a recorded event is a blocker, not a style note — it rewrites a career that has no other copy); corrections are superseding events; rulesets are versioned config and a pinned version is IMMUTABLE; the build prices but NEVER scores (ADR 0008); commits go through /commit only.
...
const CORE = [
  ...
  { key: 'correctness', schema: 'findings', mandate: `REVIEWER — CORRECTNESS / BUG HUNTER. Hunt for REAL bugs in the changed code: logic errors, off-by-one, unhandled None/empty, wrong API or argument use, broken control flow, a regression in a function the diff touched. This project's highest-value bug classes: (a) FOLD ORDER — does replaying events in a different order, or with a correction interleaved, produce a different state than it should? (b) CORRECTION RESOLUTION — does a superseding event actually supersede, and does a correction of a correction resolve to the newest? (c) RULESET RESOLUTION — does the fold price an old event against the CURRENT ruleset instead of its pinned version? That silently rewrites history and no test will notice unless one exists. (d) CAREER SCOPING — can state or cached values leak between concurrent careers? For Python, check timezone-aware datetimes and pathlib usage. Read the surrounding code, not just the diff hunk. Each finding needs a real file:line AND a concrete failure scenario (the input that breaks it) — reproduce it by running it where you can. Do not invent bugs to pad.` },
  { key: 'edgecases', schema: 'findings', mandate: `REVIEWER — TEST & EDGE-CASE. (1) Does the change carry tests for its new behavior, and do they actually exercise it (read them — a test that asserts nothing is theatre)? A fold or economy change with no pinned fixture is UNTESTED regardless of how many other tests exist. (2) ADVERSARIALLY GENERATE edge cases the implementation might mishandle, and CHECK each by running or tracing it. This project's known breakers: a DNP (zero minutes — per-36 and any rate stat must not explode or divide by zero); an overtime game (minutes no longer sum to 240); a box score CORRECTED after later games were already recorded; a correction of a correction; an event pinning a ruleset version that no longer exists on disk; two careers created with the same slug; a career with exactly one game (shrinkage at n=1); a spend that exactly exhausts the XP balance; a badge equip that exceeds budget by one. (3) Regression risk to the fold, the replay guarantee, or an existing ruleset? Ground each finding in a real input + the observed or traced behavior.` },
]

const SPEC_DEFS = {
  'ledger': { key: 'ledger', schema: 'findings', mandate: `SPECIALIST — LEDGER INTEGRITY (src/rpg_core or careers/ touched). The ledger has NO UPSTREAM: the 2K save is encrypted, so careers/<slug>/events.jsonl is the only copy of a career that will ever exist. (1) MUTATION: does any code path edit, overwrite, reorder, or delete a recorded event? BLOCKER. Corrections are SUPERSEDING APPENDS resolved by the fold (ADR 0003). (2) REPLAY DETERMINISM: RUN the replay test. Folding the same log twice must produce identical state, and a fold must not depend on wall-clock time, dict ordering, or filesystem order. (3) VERSION PINNING: does every appended event record the ruleset version live at the time, and does the fold resolve each event against ITS pinned version rather than the current one? A fold that reads the live ruleset silently re-scores history. (4) WRITE SAFETY: is the append atomic, and can a crash mid-write corrupt the file or leave a partial line? (5) TESTS AGAINST FIXTURES: does any test read or write a real career under careers/? BLOCKER — fixtures live in tests/fixtures/. Ground each finding with file:line.` },
  'economy': { key: 'economy', schema: 'findings', mandate: `SPECIALIST — ECONOMY & RULESETS (rulesets/, pricing, or the earn model touched). (1) COST-SIDE ONLY: does vitals or archetype leak into the SCORING model anywhere? ADR 0008 forbids it — the build prices upgrades and NEVER scores production, including sympathetic versions like crediting a rim protector for blocks. BLOCKER. (2) RULES-IN-CODE: is any tunable number (an XP coefficient, a cost exponent, an affinity multiplier, a badge budget, a milestone threshold) hardcoded in Python instead of living in a ruleset file? ADR 0004. Each one silently breaks replay for careers that predate it. (3) IMMUTABLE VERSIONS: does the change EDIT an existing ruleset version that recorded events already pin, rather than creating a new one? BLOCKER — that rewrites history. (4) EXPECTATION SANITY: does the earn model degrade gracefully at low minutes (shrinkage present, no divide-by-zero on a DNP, no per-36 explosion)? (5) FIXTURES: is there a pinned expectation fixture, so a retune cannot silently undo a previous calibration decision? Ground each with file:line.` },
  'app': { key: 'app', schema: 'findings', mandate: `SPECIALIST — API & WEB APP (the API or frontend touched). (1) CRUD OVER AN APPEND-ONLY LOG: does any endpoint offer an update or delete that MUTATES rather than appending a superseding event? BLOCKER — this is the single easiest way to destroy the architecture. (2) MULTI-CAREER: does anything assume a single career — a module-level singleton, a default career id, a route missing its career scope, shared mutable state between careers? (3) DOMAIN LEAKAGE: does economy or ledger logic live in a route handler or a component instead of src/rpg_core? The core must stay I/O-free and web-free or it stops being testable in isolation. (4) VALIDATION AT THE BOUNDARY: is the BoxScore DTO validated where it enters (ADR 0002), rather than trusting request shape downstream? (5) TRANSCRIPTION SAFETY: is the worksheet unambiguous — every change showing old→new, nothing truncated, nothing requiring the user to infer a value? A misread worksheet desyncs the app from the game with NO way to detect it (ADR 0001). Ground each with file:line.` },
  'builder': { key: 'builder', schema: 'findings', mandate: `SPECIALIST — DATASET BUILDER (build/ or datasets/ touched). (1) RESOLVE BY NAME: does every consumer resolve through datasets/manifest.json rather than a literal path or a parents[N] walk? Is the new dataset registered with its provenance? (2) CACHING: does a re-run hit the network again, or does it read var/cache/ with an explicit --fetch to re-pull? An expensive pull that repeats on every run will get disabled and then rot. (3) EPISTEMICS: does any code or doc assert a source's shape that NOBODY has actually pulled? An unconfirmed belief presented as fact is a finding — label it and make it a task. (4) TESTS AGAINST FIXTURES: do the builder's tests run against a committed trimmed payload, never the live source? A network-dependent test in CI is a BLOCKER. (5) NO BULK DATA IN GIT: is a large pull landing in datasets/ when it belongs in var/cache/? Ground each with file:line.` },
  'skill-quality': { key: 'skill-quality', schema: 'findings', mandate: `SPECIALIST — SKILL QUALITY (.claude/skills touched; the feature is or edits a skill). (1) Does SKILL.md have valid frontmatter (name + a trigger-rich description), and does the DESCRIPTION match what the skill now does — a drifted description is a triggering bug? (2) Progressive disclosure — is the body reasonably scoped, with heavy logic in a sibling script rather than inlined? (3) Is the skill registered where the repo expects: CLAUDE.md "How to help", and — for a pipeline skill — the appropriate track README (requests/feature-requests/README.md, requests/bugfix-requests/README.md, or requests/calibration-findings/README.md)? (4) Does it honor the conventions: read-only-git subagents, resolve-by-name, agents-never-commit? (5) Do its relative links resolve? tests/test_request_links.py is the mechanical check — RUN it. Flag each with the file:line.` },
  'infra-cost': { key: 'infra-cost', schema: 'findings', mandate: `SPECIALIST — CI, SECRETS & REPO HYGIENE (CI or project config touched). (1) SECRETS: this repo is PUBLIC. Does the diff introduce any credential, token, account id, personal email, or machine-specific absolute path in a tracked file? Check that new config reads from env vars rather than literals and that .env stays gitignored. A leaked secret is a BLOCKER. (2) GITIGNORE INVARIANTS: does any new ignore rule shadow the careers/ carve-out? careers/**/events.jsonl MUST stay tracked — a blanket *.jsonl rule silently stops backing up the only copy of every career. Conversely, var/ must stay ignored. tests/test_repo_structure.py checks both — RUN it. (3) CI INTEGRITY: does the workflow actually FAIL on the condition it claims to check? A step that passes vacuously (an empty selection, a swallowed exit code, a skipped leg) is worse than no check. Read the workflow, not the step names. (4) BRANCH PROTECTION: if a CI job display name changed, was ops/branch-protection.json updated in the SAME commit? A rename silently makes PRs wait forever for a check that never reports. (5) REPRODUCIBILITY: is the lockfile updated alongside pyproject.toml? Ground each finding.` },
}
const AREA_TO_SPEC = {
  src: ['ledger', 'economy'], rulesets: ['economy'], app: ['app'],
  datasets: ['builder'], build: ['builder'], tests: ['ledger'],
