...
description: >-
  ...
  requests/calibration-findings/README.md).
---
...
- an idea that **plainly collides with a hard constraint**. The common ones here: the data
  simply isn't readable (anything in the encrypted save tree — see
  [`docs/data-access.md`](../../../docs/data-access.md) and ADR 0001), or the idea collides with a
  settled ADR (archetype in the scoring model, athletic regression, a training subsystem). Roadmap
  items are pre-scoped; start there. Don't silently write a request for a dead-on-arrival idea: name the specific
...
- Which subsystem? Domain core (`src/rpg_core/`), the API or web app, a dataset builder
  (`build/` → `datasets/`), rulesets (`rulesets/`),
