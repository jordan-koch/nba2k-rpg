...
const SHARED = `You are scoping a proposed new feature for an RPG progression engine for NBA 2K26 MyNBA player-locked careers: box scores are entered by hand, an expectation-relative economy converts them to currency, and the player spends it on attributes and badges priced by their build. Python domain core (src/rpg_core) + FastAPI + React. Careers are an append-only event ledger folded into state. Read docs/decisions/ before proposing anything structural — nine ADRs are settled and re-litigating them is expensive. Your working directory is the repo root.
...
- CLAUDE.md — the project rules you must scope within: resolve datasets BY NAME via datasets/manifest.json (never a literal path); the LEDGER IS APPEND-ONLY and corrections are superseding events; rulesets are versioned config and a pinned version is IMMUTABLE; the build prices upgrades but NEVER scores production; commits go through /commit only; subagents get read-only git
- docs/data-sources.md — what the source actually provides, from which season, and what breaks. NOTE: its contents are currently marked unconfirmed, so treat anything there as a claim to verify, not a fact to build on
- docs/decisions/ — the ADRs. A scope that contradicts an accepted ADR must say so explicitly rather than quietly diverging

IF THIS FEATURE TOUCHES A DATASET, the scope must settle five contracts: grain (one row per WHAT), keys, coverage (which seasons the source actually has), update semantics, and pull cost (how many requests, at what pacing, what gets cached so a re-run is free). IF IT TOUCHES THE ECONOMY OR THE LEDGER, it must instead settle: which ruleset version changes, whether existing careers replay, and whether replaying shifts recorded history.
...
const SCOPERS = [
  { key: 'fit', label: 'repo-fit',
    mandate: `SCOPER 1 — REPO-FIT & ARCHITECTURE. Scope this feature through the lens of how it fits THIS repo. Decide: does it belong here, and how does it integrate with the existing architecture? Name the concrete modules, datasets (by logical name), rulesets, and skills it leverages, extends, or joins to; whether it belongs in the I/O-free domain core, the API, the app, or a builder, and why; the conventions that constrain it (resolve-by-name, append-only ledger, corrections-are-appends, rulesets-versioned-and-pinned); the integration seams; and where it would conflict with or duplicate something that already exists. Render a frank FIT verdict — clean fit / awkward fit needing reshaping / poor fit — with grounded reasons. Propose the scope that goes with the grain of the repo: goals, non-goals, the in-scope core, and acceptance criteria expressed against real repo artifacts.` },
