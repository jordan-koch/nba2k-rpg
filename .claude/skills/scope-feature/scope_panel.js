...
const SHARED = `You are scoping a proposed new feature for an NBA data platform — an Iceberg lakehouse with a dbt medallion architecture (bronze/silver/gold), extracting from stats.nba.com, covering seasons 2003-04 onward. It is deliberately built past what its data volume requires; see docs/decisions/0001-deliberate-over-engineering.md before calling something overkill. Your working directory is the repo root.
...
- requests/feature-requests/README.md — the pipeline contract: the PROJECT_SCOPE.md handoff interface this scope must satisfy, the two standing principles GREEDY-BUT-GATED and GENERATE -> CONVERGE -> TRIAGE -> YOU-DECIDE, and what "testable acceptance criteria" means for data work (a dbt test that passes or fails, not a number a human eyeballs)
- CLAUDE.md — the project rules you must scope within: resolve BY NAME (dbt ref()/source(), never a literal path); the landing zone is IMMUTABLE; bronze is 1:1 with the source; silver declares its grain AND proves it with a uniqueness test; facts MERGE on key because box scores get restated; commits go through /commit only; subagents get read-only git
...
IF THIS FEATURE TOUCHES A DATASET, the scope must settle five contracts: grain (one row per WHAT), keys, era coverage (which seasons the source actually has, and which columns are structurally absent before which season), update semantics (append vs merge-on-key), and extraction cost (how many API calls, at what pacing, over what wall-clock).
...
const SCOPERS = [
  { key: 'fit', label: 'repo-fit',
    mandate: `SCOPER 1 — REPO-FIT & ARCHITECTURE. Scope this feature through the lens of how it fits THIS repo. Decide: does it belong here, and how does it integrate with the existing architecture? Name the concrete models (by ref name), sources, modules, and skills it leverages, extends, or joins to; which medallion layer each piece belongs in and why; the conventions that constrain it (resolve-by-name, immutable landing zone, bronze-is-1:1, grain-declared-and-tested, merge-on-key); the integration seams; and where it would conflict with or duplicate something that already exists. Render a frank FIT verdict — clean fit / awkward fit needing reshaping / poor fit — with grounded reasons. Propose the scope that goes with the grain of the repo: goals, non-goals, the in-scope core, and acceptance criteria expressed against real repo artifacts.` },
