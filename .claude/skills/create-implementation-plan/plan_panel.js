...
const SHARED = `You are turning a DECIDED upstream artifact into an IMPLEMENTATION PLAN a COLD agent can execute without the author present — for an RPG progression engine for NBA 2K26 MyNBA player-locked careers: box scores are entered by hand, an expectation-relative economy converts them to currency, and the player spends it on attributes and badges priced by their build. Python domain core (src/rpg_core) + FastAPI + React. Careers are an append-only event ledger folded into state. Working directory = repo root.
...
- CLAUDE.md — the conventions the PLAN must BAKE IN for the cold implementer: resolve datasets BY NAME via datasets/manifest.json (never a literal path); the LEDGER IS APPEND-ONLY and corrections are superseding events, never mutations; rulesets are versioned config and a version any recorded event pins is IMMUTABLE; the build prices upgrades but NEVER scores production (ADR 0008); careers/ is tracked in git because the encrypted save means there is no upstream; commits go through /commit only (never ad hoc, never merge/amend; /commit pushes the branch, the PR stays the user's); anything that spends cloud money is user-run; subagents get read-only git.
