...
const SHARED = `You are turning a DECIDED upstream artifact into an IMPLEMENTATION PLAN a COLD agent can execute without the author present — for an NBA data platform: an Iceberg lakehouse with a dbt medallion architecture (bronze/silver/gold), extracting from stats.nba.com, covering seasons 2003-04 onward. Working directory = repo root.
...
- CLAUDE.md — the conventions the PLAN must BAKE IN for the cold implementer: resolve BY NAME (dbt ref()/source(), never a literal path); the landing zone is IMMUTABLE; bronze is 1:1 with the source; silver declares its grain AND proves it with a uniqueness test; facts MERGE on key; tracking columns are structurally absent before 2013-14; commits go through /commit only (never ad hoc, never merge/push/amend); anything that spends cloud money is user-run; subagents get read-only git.
- docs/data-sources.md — source coverage and known gotchas. Its contents are currently marked unconfirmed; a plan that depends on an unconfirmed claim must include a phase that VERIFIES it before the phases that build on it.

GROUND EVERY CLAIM in concrete repo artifacts — cite REAL files / line numbers / function names that you have actually read. Do NOT invent a path or a line number: a cold implementer trusts them literally, so a wrong citation is worse than none. The plan PRESCRIBES a per-phase cadence where each phase ends at a /commit-gated checkpoint, on a green local run (uv run pytest, plus uv run dbt build --target ci when models changed).
...
const PLANNERS = [
  ...
  { key: 'sequencing', mandate: `PLANNER 2 — SEQUENCING, TESTABILITY & PHASING. Draft the plan as an ORDERED sequence of independently-verifiable phases, each with (a) a crisp goal, (b) concrete steps, (c) its own ACCEPTANCE criteria that actually verify the phase, and (d) a checkpoint note (each phase ends green locally and is handed to the user to commit). Make each phase shippable and reversible; order to de-risk — foundational and highest-uncertainty work first. CRITICAL FOR THIS REPO: any phase that depends on an UNCONFIRMED claim about a source endpoint (its shape, its coverage, its cost) must be preceded by a phase that VERIFIES that claim against a real response, because docs/data-sources.md is currently a catalog of beliefs rather than facts. Specify how each phase is verified (which pytest test, which dbt test) and the regression safety.` },
  { key: 'domain-convention', mandate: `PLANNER 3 — DATA-MODEL & CONVENTION CORRECTNESS (auto-scaling). IF the feature touches data: ground the plan in the five contracts — grain (one row per WHAT), keys, era coverage (which seasons the source has; which columns are structurally absent before 2013-14), update semantics (merge-on-key, because box scores get restated), and extraction cost. Name the medallion layer each piece belongs in and why, and specify the uniqueness test that will PROVE each declared grain. Flag anywhere player-team affiliation must resolve as-of the game date rather than as-of today — that is this project's most likely silent-wrongness bug. IF it is a feature with thin/no data surface (a skill, CI, infrastructure): PIVOT this lens to PROJECT-CONVENTION correctness — make sure the plan honors and BAKES IN resolve-by-name, the immutable landing zone, agents-never-commit, read-only-git subagents, user-run-for-anything-billable, and the established skill patterns, so the cold implementer can't violate CLAUDE.md by following it. Do not force irrelevant data commentary onto a change that touches no data.` },
]
