...
<the CLAUDE.md rules the implementer must honor: resolve datasets by logical name (never a literal
path); corrections are appends and recorded events are never mutated; rulesets are versioned config,
and a version any event pins is immutable; the build prices but never scores; commits go through /commit only;
subagents get read-only git. Add the cost rule only when the change can spend cloud money. Don't force
irrelevant conventions onto a change that doesn't touch the ledger or the economy.>
